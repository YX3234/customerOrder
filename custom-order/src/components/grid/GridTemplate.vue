...
const { dragData, useBoxSize, useBoxGrid, booleanWithin, booleanIntersects, ceil } = useDragStore();
...
const onDragenter = (e: DragEvent) => {
    e.preventDefault();
    const dragDataInstance = dragData.get(props.groupName);
    if (dragDataInstance) {
        current.column = dragDataInstance.column || 0;
        current.row = dragDataInstance.row || 0;
        current.x = getX(e.offsetX) - getX(dragDataInstance.offsetX ?? 0);
        current.y = getY(e.offsetY) - getY(dragDataInstance.offsetY ?? 0);
        current.id = dragDataInstance.id;
        current.show = true;
    }
};
...
const onDrop = async (e: DragEvent) => {
    e.preventDefault();
    current.show = false;
    const dragDataInstance = dragData.get(props.groupName); // JSON.parse(e.dataTransfer.getData('application/json'))
    if (dragDataInstance &&
        isPutDown.value &&
        ...
        if (dragDataInstance?.id) {
            dragDataInstance.x = current.x;
            dragDataInstance.y = current.y;
        } else {
            list.value.push({
                ...dragDataInstance,
                x: current.x,
...
const onResizeStart = () => {
    const dragDataInstance = dragData.get(props.groupName);
    if (dragDataInstance) {
        current.column = dragDataInstance.column;
        current.row = dragDataInstance.row;
        current.x = dragDataInstance.x;
        current.y = dragDataInstance.y;
        current.id = dragDataInstance.id;
        current.show = true;
    }
};
...
const onResizeEnd = async () => {
    ...
    if (dragDataInstance &&
        ...
        dragDataInstance.column = current.column;
        dragDataInstance.row = current.row;
    }
};
...
<template>
    <div ref="designerContentRef" class="drop-content">
        <div class="drop-content__drop-container" @dragenter="onDragenter($event)" @dragover="onDragover($event)"
